...
// File contracts/core/storage/TimelockStorage.sol
...
contract Timelock is TimelockStorage {
    ...
    function setTokenConfig(address _vault, address _token, uint256 _tokenWeight, uint256 _minProfitBps, uint256 _maxZkusdAmount, uint256 _bufferAmount, uint256 _zkusdAmount) external onlyKeeperAndAbove {
        ...
        IVault(_vault).setZkusdAmount(_token, _zkusdAmount);
    }
    ...
    function signalVaultSetTokenConfig(address _vault, address _token, uint256 _tokenDecimals, uint256 _tokenWeight, uint256 _minProfitBps, uint256 _maxZkusdAmount, bool _isStable, bool _isShortable) external onlyAdmin {
        ...
        _setPendingAction(action);
        emit Events.SignalVaultSetTokenConfig(_vault, _token, _tokenDecimals, _tokenWeight, _minProfitBps, _maxZkusdAmount, _isStable, _isShortable);
    }
    function vaultSetTokenConfig(address _vault, address _token, uint256 _tokenDecimals, uint256 _tokenWeight, uint256 _minProfitBps, uint256 _maxZkusdAmount, bool _isStable, bool _isShortable) external onlyAdmin {
        bytes32 action = keccak256(abi.encodePacked(
                "vaultSetTokenConfig",
                _vault,
                _token,
        ...
        _validateAction(action);
        _clearAction(action);
        IVault(_vault).setTokenConfig(_token, _tokenDecimals, _tokenWeight, _minProfitBps, _maxZkusdAmount, _isStable, _isShortable);
    }

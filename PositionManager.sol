...
// File contracts/core/storage/BasePositionManagerStorage.sol
...
// File contracts/core/settings/BasePositionManagerAggregator.sol
...
// File contracts/core/settings/BasePositionManagerSettings.sol
...
// File contracts/core/storage/PositionManagerStorage.sol
...
// File contracts/core/settings/PositionManagerAggregator.sol
...
// File contracts/core/settings/PositionManagerSettings.sol
...
contract PositionManager is BasePositionManager, PositionManagerSettings {
    constructor(
        address _vault, address _router,
        address _shortsTracker, address _weth,
        uint256 _depositFee, address _orderBook)
    public BasePositionManager(_vault, _router, _shortsTracker, _weth, _depositFee) {
        orderBook = _orderBook;
    }
    
    function increasePosition(
        address[] memory _path, address _indexToken,
        uint256 _amountIn, uint256 _minOut,
        uint256 _sizeDelta, bool _isLong, uint256 _price)
    external nonReentrant onlyPartnersOrOpened {
    ...
    function increasePositionETH(
        address[] memory _path, address _indexToken,
        uint256 _minOut, uint256 _sizeDelta,
        bool _isLong, uint256 _price)
    ...
    function decreasePosition(
        ...
        uint256 _price, uint256 _minOut, bool _withdrawETH)
    external nonReentrant onlyPartnersOrOpened {
        require(_path.length == 1 || _path.length == 2, Errors.POSITIONMANAGER_INVALID_PATH_LENGTH);
    ...
    function liquidatePosition(
        address _account, address _collateralToken,
        address _indexToken, bool _isLong, address _feeReceiver)
